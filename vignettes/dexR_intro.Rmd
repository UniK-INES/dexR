...
`dexR` relies on a CSV configuration table which contains links to CSV tables used to derive the XML configuration for the EMG. The package contains template files which are copied to `dexpa$dirs$config` (default: `./config`) when running

```{r, eval=FALSE}
dexR::setup_environment(dexpa)
```
This routine also copes `DEX_Runs.csv` which is used to store run information.
...
5. Start market backend
6. Generate EMG XML configuration
7. Start EMG client(s)
