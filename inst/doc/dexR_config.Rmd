...
To run the smaple project, call:
...
As explained in [Intro](dexR_intro.html#Parameters), configuration of dexR is done on four levels (project > machine > project version > simulation). Files for the first two levels need to be copied to project-specifc location once for a project:
...
```
No it's time to adapt `dexpa-project.R` and `dexpa-machine_machine.R` to your needs.
...
```
You can now adapt ```<project>/scripts/<version>/dexpa-pversion_<version>.R```
